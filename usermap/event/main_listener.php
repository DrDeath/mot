...
/*		// check whether location, zip code or country have been altered
		$query = 'SELECT pf_phpbb_location, pf_mot_zip, pf_mot_land
				FROM ' . PROFILE_FIELDS_DATA_TABLE . '
				WHERE user_id = ' . (int) $user_id;
		$result = $this->db->sql_query($query);
		$row = $this->db->sql_fetchrow($result);
		$this->db->sql_freeresult($result);
		if (($cp_data['pf_phpbb_location'] <> $row['pf_phpbb_location']) or ($cp_data['pf_mot_zip'] <> $row['pf_mot_zip']) or ($cp_data['pf_mot_land'] <> $row['pf_mot_land']))
		{
			// Something of interest for us has been altered, now we have to work with it */
...
/*		}
		// if nothing has been altered we are already done */
	}
